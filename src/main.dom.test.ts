...
let results: HTMLTableSectionElement
...
function getCsvRow(filePath: string, row: number) {
  let targetCsv: string[][] = new Array()
  switch (filePath) {
    case 'cats':
...
test('mode', () => {
  ...
  expect(results.rows[index++].textContent).toBe(
    'Output: Switched to verbose output mode.'
  )
  runCommand('mode')
  expect(main.isVerboseMode).toBe(false)
...
test('view', () => {
  ...
  for (let r = 0; r < exampleCSVs.cats.length; r++) {
  ...
  for (let r = 0; r < exampleCSVs.stars.length; r++) {
  ...
  for (let r = 0; r < exampleCSVs.students.length; r++) {
  ...
  for (let r = 0; r < exampleCSVs.burgers.length; r++) {
...
test('search', () => {
  ...
  expect(results.rows[index++].textContent).toBe(
    '[Error] Too many search terms provided.'
  )
  // only value given
...
test('query', () => {
  ...
  index += 2
  runCommand(
    `query AND('Lettuce, Tomato, Onion, Pickles' in 'Toppings', '900' in Calories')`
  )
